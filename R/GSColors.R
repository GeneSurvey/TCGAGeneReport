...
{
	size <- 27
	tcgaColors <- getColorList(size)
	message(paste(tcgaColors, collapse=", "))

	x <- 20*(1:length(tcgaColors))
	y <- 20*(1:length(tcgaColors))
	pie(rep(1,length(tcgaColors)), col = tcgaColors, labels=tcgaColors)
...
getColorsForDiseases <- function(theDiseaseList)
{
	...
	if (length(newDiseases)>0)
	{
		...
		returnColors <- newColorSet[order(names(newColorSet))]
	}
	returnColors
}
